...
import { FileModelFactory, FileViewModelFactory } from './model-factories';
...
export class FilesService implements IFilesService {
  ...
  public async uploadFiles(urls: string[]): Promise<FileViewModel[]> {
    try {
      const results = await Promise.all(
        urls.map((url) => this.handleFileProcessing(url)),
      );
  ...
  public async getAllFiles(): Promise<FileViewModel[]> {
    const files = await this.filesRepository.getAll();
  ...
  private async handleFileProcessing(url: string): Promise<FileEntity> {
    const { stream: downloadStream, size: fileSize } =
      await this.downloadFileStream(url);
    ...
    downloadStream.on('end', () => {
      this.logger.log('File downloading completed.');
    });

    const metadata = await this.extractFileMetadata(url, downloadStream);
    const { gDriveId } = await this.googleDriveService.uploadFile(
      downloadStream,
      metadata.fileName,
    );
  ...
  private async downloadFileStream(
    ...
    try {
      ...
      if (isNaN(totalLength)) {
        this.logger.warn(
          'Failed to retrieve file size from Content-Length header.',
        );
      } else {
        this.logger.log(
          `Retrieved file size: ${(totalLength / (1024 * 1024)).toFixed(2)} MB`,
        );
      }
  ...
  private async extractFileMetadata(
    ...
    try {
      ...
      const contentLength = parseInt(
        (fileStream as any).headers?.['content-length'] || '0',
        10,
      );
      ...
      this.logger.error(
        `Failed to extract file metadata from URL: ${url}`,
        error.stack,
      );
      throw new HttpException(
  ...
  private async saveFileMetadata(
    ...
    try {
      const newEntity = FileModelFactory.create({
      ...
      return await this.filesRepository.create(newEntity);
    } catch (error) {
      this.logger.error(
        `Failed to save file metadata for URL: ${url}`,
        error.stack,
      );
