...
export class GoogleDriveService implements IGoogleDriveService {
    ...
    constructor(private readonly configService: ConfigService) {
        ...
        this.googleDriveFolderId = this.configService.get<string>('google.gDriveFolderId');
    ...
    public async uploadFile(stream: Readable, fileName: string): Promise<GoogleDriveFile> {
        try {
    ...
    private async setFilePublic(fileId: string): Promise<drive_v3.Schema$Permission> {
        try {
